 ...
 * center for the search. To clear this value, set it to `kCLLocationCoordinate2DInvalid`. */
@property(nonatomic, assign) CLLocationCoordinate2D center;

/* Bitwise ORs of different viewing options to display in the Google Maps application. @see GoogleMapsViewOptions */
...
/* Helper class used to define a Street View location to be opened in Google Maps, Currently, this class is a subset of 
 * GoogleMapDefinition, but we're keeping these two classes separate in case they diverge in the future. */
...
/* Query string to use to determine this waypoint. For best results, use unambiguous strings such as addresses
 * ("355 Main Street, Cambridge, MA") or business search queries with unique results ("The Exploratorium, San Francisco CA"). */
@property(nonatomic, copy) NSString *queryString;
@end

/** Helper class used to define two waypoints from which to create a set of directions in Google
 *  Maps. Either the starting point or the destination must be non-nil in order to produce a valid request.*/
@interface GoogleDirectionsDefinition : NSObject
/* Starting point. If this is set to `nil`, we will start at the user's current location. @see GoogleDirectionsWaypoint */
@property(nonatomic, strong) GoogleDirectionsWaypoint *startingPoint;
/* Destination. If this is set to `nil`, we will end at the user's current location. @see GoogleDirectionsWaypoint*/
@property(nonatomic, strong) GoogleDirectionsWaypoint *destinationPoint;
/*  Method of transportation for which to provide directions. If the application opened does not support this travel type,
 *  it will fall back to providing driving directions. @see GoogleMapsTravelMode */
...
/* Singleton method. Use this for making any calls against the `OpenInGoogleMapsController` class. @return Returns the shared singleton instance.*/
+ (MapViewController *)sharedInstance;
/* The callback URL that you want Google maps (or Google Chrome) to use to redirect back to your application.
 * If `x-callback-url` is not supported, this will not be used and Google Maps will be opened with a simple `comgooglemaps:`
 * call instead.See the Google Maps URL scheme documentation for more information about `x-callback-url` support. */
@property(nonatomic, strong) NSURL *callbackURL;

/* Determines what to do if the user does not have Google Maps. You only need to set the fallback strategy once during the
 * lifetime of your application and it will be used in all future `OpenInGoogleMapsController` requests. @see GoogleMapsFallback*/
